  ...
  return (
    <div className="bg-slate-200 p-3 flex gap-4 border border-slate-400">
      ...
      <div className='flex justify-between'>
        ...
        {/**delete product */}
        <div className='cursor-pointer text-lg text-slate-800 hover:text-red-400'onClick={()=>dispatch(deleteCartItems(id))}>
          <AiFillDelete/>
          </div>
        </div>
        ...
        <div className='flex justify-between'>
          ...
          <div className="flex items-center gap-2 font-bold text-slate-400">
            <p>Total:</p>
            <p className='text-red-400'>{total}<span className='text-red-500'>$</span></p>
          </div>
        </div>
      </div>
    </div>
